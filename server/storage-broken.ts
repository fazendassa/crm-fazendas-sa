...
import { db } from "./db";
import { eq, ilike, and, count, desc, ne, sql, isNotNull, sum, or } from "drizzle-orm";
...
export interface IStorage {
  ...
  importContacts(contacts: InsertContact[]): Promise<{ success: number; errors: string[] }>;
  getAvailableTags(): Promise<string[]>;
  createContactsFromImport(data: any[], pipelineId?: number, tags?: string[], fieldMapping?: any): Promise<{ success: number; errors: string[] }>;
  ...
  // WhatsApp Message operations
  getWhatsappMessages(sessionId?: number, phoneNumber?: string, limit?: number, offset?: number): Promise<WhatsappMessageWithRelations[]>;
  createWhatsappMessage(message: InsertWhatsappMessage): Promise<WhatsappMessage>;
  updateWhatsappMessage(id: number, message: Partial<InsertWhatsappMessage>): Promise<WhatsappMessage>;
}
...
class DatabaseStorage implements IStorage {
  ...
  // WhatsApp Message operations
  async getWhatsappMessages(sessionId?: number, phoneNumber?: string, limit = 100, offset = 0): Promise<WhatsappMessageWithRelations[]> {
    const baseQuery = db
      .select({
  ...
  async updateWhatsappMessage(id: number, message: Partial<InsertWhatsappMessage>): Promise<WhatsappMessage> {
    const [updatedMessage] = await db
      ...
      .returning();
    return updatedMessage;phoneNumber, phoneNumber));
    return contact;
  }
